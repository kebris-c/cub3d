...
/*
** Loads a scene file into a NULL-terminated char ** using get_next_line.
** Uses open/read-style POSIX I/O (subject-friendly) in two passes: count,
** allocate, then fill.
*/
...
static int	read_lines_into_table(const char *path, char ***lines, int total)
{
	int	fd;
	...
	if (fd < 0)
	{
		...
		*lines = NULL;
		perror("cub3d");
		return (error_msg("cannot reopen .cub file"));
	}
...
int	load_file_lines(const char *path, char ***lines)
{
	...
	*lines = ft_calloc((size_t)total + 1, sizeof(char *));
	...
	return (read_lines_into_table(path, lines, total));
}
